...
class LamdaResampleLoss(nn.Module):
    ...
    def __init__(self,
        ...
        super(LamdaResampleLoss, self).__init__()
    ...
    # def co_relate(self, inputs, factor):
    #     logits_softmax = F.softmax(inputs, dim=1)
    #     logits_sigmoid = F.sigmoid(inputs)
    #     loss_mutual = factor * F.kl_div(logits_softmax, logits_sigmoid,reduction='batchmean')
    ...
    def logit_reg_functions(self, labels, logits, weight=None):
        ...
        new_logist = F.sigmoid(logits.clone()) 
        if 'init_bias' in self.logit_reg:
            logits += self.init_bias
        if 'neg_scale' in self.logit_reg:
            logits =  logits * (1 - labels) * (10 * (1-new_logist) + 1)  +  logits * labels
            weight = weight / (10 * (1-new_logist.detach()) + 1)* (1 - labels) +  weight * labels
        return logits, weight
