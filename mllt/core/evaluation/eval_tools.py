...
# tpr =  tp / (tp + fn)
# tnr = tn / (fp + tn )
def eval_bacc(results, gt_labels):
    y_pred = np.asarray(results) > 0
    tp = gt_labels * y_pred
    tn = (1 - gt_labels) * (1 - y_pred)
    fp = (1 - gt_labels) * y_pred
...
def eval_auc(results, gt_labels):
    macro_auc = roc_auc_score(gt_labels, sigmoid(results) , average="macro",)
    micro_auc = roc_auc_score(gt_labels, sigmoid(results),  average="micro",)
    weighted_auc = roc_auc_score(gt_labels, sigmoid(results),  average="weighted")
    per_auc = roc_auc_score(gt_labels, sigmoid(results),  average=None)
